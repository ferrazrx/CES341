...
export class PhonesRepository implements Repository<Phone> {
  ...
  async all(): Promise<Phone[]> {
    try {
      return await PhoneModel.find({}).catch((e) => {
        console.error(e);
  ...
  async createOne(phone: Phone): Promise<Phone> {
    try {
      return await PhoneModel.create(phone).catch((e) => {
        console.error(e);
        throw new DatabaseError('Something went wrong!');
      });
    } catch (e) {
  ...
  async findByID(_id: string): Promise<Phone | null> {
    try {
      return await PhoneModel.findById(_id).catch((e) => {
        console.error(e);
        throw new DatabaseError('Something went wrong!');
      });
    } catch (e) {
  ...
  async updateOne(_id: string, data: Phone): Promise<Phone | null> {
    try {
      ...
      return await this.findByID(_id).catch((e) => {
        console.error(e);
        throw new DatabaseError('Something went wrong!');
      });
    } catch (e) {
  ...
  async deleteOne(_id: string): Promise<Acknowledge> {
    try {
      const result = await PhoneModel.deleteOne({ _id }).catch((e) => {
        console.error(e);
        throw new DatabaseError('Something went wrong!');
      });
